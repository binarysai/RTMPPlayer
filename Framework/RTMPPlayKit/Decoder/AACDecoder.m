...
- (NSData *)decodeWithAACBuf:(void *)srcdata aacLen:(int)srclen {
    if (_audioConverter) {
        ...
        if (_outBuffer == NULL){
            _outBuffer = malloc(theOuputBufSize);
        }
        ...
        outputBuffers.mBuffers[0].mData = _outBuffer;
        ...
        if (ret == noErr) {
            if (outputBuffers.mBuffers[0].mDataByteSize > 0) {
                pcmData=  [NSData dataWithBytes:outputBuffers.mBuffers[0].mData length:outputBuffers.mBuffers[0].mDataByteSize];
            }
        }else{
